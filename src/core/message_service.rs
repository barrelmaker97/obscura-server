...
impl MessageService {
    ...
    pub async fn enqueue_message(&self, sender_id: Uuid, recipient_id: Uuid, content: Vec<u8>) -> Result<()> {
        ...
        self.repo.create(sender_id, recipient_id, content, self.config.message_ttl_days).await?;
        Ok(())
    }

    /// Periodically cleans up expired messages and enforces inbox limits.
    pub async fn run_cleanup_loop(&self) {
        let mut interval = tokio::time::interval(Duration::from_secs(self.config.message_cleanup_interval_secs));
        ...
        loop {
            ...
            match self.repo.delete_global_overflow(self.config.max_inbox_size).await {
                Ok(count) => {
                    if count > 0 {
